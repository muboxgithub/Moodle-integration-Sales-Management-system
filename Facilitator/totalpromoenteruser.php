...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['promoid'])) {
        ...
        $getChildIds=getTotalChildPromoCode($conn3,$promoid);

        $promoids=array_merge($promoid,$getChildIds);
        ...
        $sqlapplied = "SELECT COUNT(u.id) AS totenterpromocodeuser, u.username, ind.data FROM `mdl_user` u
JOIN `mdl_user_info_data` ind 	ON ind.userid=u.id
